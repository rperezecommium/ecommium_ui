...
function PurchaseCard({ purchase }: { purchase: StorefrontPurchase }) {
  const primaryItems = purchase.items.slice(0, 3);
    ...
    <article className="storefrontPurchaseCard">
      <div className="storefrontPurchaseHeader">
        <div>
          ...
          <h3>{primaryItems[0]?.name ?? "Compra registrada"}</h3>
      ...
      {purchase.shipping ? (
        <div className="storefrontPurchaseShipping">
          <Truck aria-hidden="true" size={18} />
          <div>
            <strong>{shippingStatusLabels[purchase.shipping.status] ?? purchase.shipping.status}</strong>
            <p>
              {[purchase.shipping.carrier.label, purchase.shipping.trackingNumber, purchase.shipping.shippingEstimate]
                .filter(Boolean)
                .join(" · ") || "Seguimiento no disponible"}
            </p>
          </div>
          {purchase.shipping.isTrackingAvailable && purchase.shipping.trackingUrl ? (
            <a href={purchase.shipping.trackingUrl} rel="noreferrer" target="_blank">
              Tracking
